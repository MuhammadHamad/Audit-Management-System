-- Fix/demo hardening for force_escalate_capa RPC
-- - Allow callers based on either public.users.role OR user_roles (via has_role)
-- - Explicitly grant execute to authenticated so PostgREST can call it
